...
int main(){
    ...
    vector<ll> tamSubArbolesHijos;
    ll total = 0;
    for( int hijo :domTree.t[1] ){
        tamSubArbolesHijos.pb(   domTree.tam_sub[hijo]  );
        total += domTree.tam_sub[hijo];
    }
    int hijosRaiz  = tamSubArbolesHijos.size();
    // if(  hijosRaiz< 2 ){
    //     cout << "\n"; 
    //     return 0;
    // }
    ...
    ll ans = 0;
    for( int i=0; i<hijosRaiz; i++){
        int tamSubArbol = tamSubArbolesHijos[i];
        DEBUG cout << tamSubArbol << " \n"[i==hijosRaiz-1];
        total -= tamSubArbol;
        ans += total * tamSubArbol;  
        
    }
    
    // las parejas (1,u)
    ans += (domTree.tam_sub[1]-1);
